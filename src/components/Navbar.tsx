  ...
  return <>
    ...
    <header className="sticky top-0 z-50 px-3 pt-3 sm:px-4">
      <div className={`container-page flex min-h-16 items-center justify-between rounded-2xl border px-3 py-2 transition-all duration-200 ${scrolled ? "border-white/10 bg-brand-dark/96 shadow-[0_18px_45px_-30px_rgba(7,38,22,.72)] backdrop-blur-xl" : "border-white/10 bg-brand-dark/88 backdrop-blur-lg"}`}>
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Primary navigation">{links.map((link) => { const active = pathname === link.to; return <Link key={link.to} href={link.to} aria-current={active ? "page" : undefined} className={`rounded-lg px-3 py-2 text-sm font-bold transition-colors ${active ? "bg-white/10 text-white" : "text-white/62 hover:bg-white/6 hover:text-white"}`}>{link.label}</Link>; })}</nav>
        <div className="hidden items-center gap-2 lg:flex"><Link href="/portal/login" className="btn-ghost-light min-h-10 px-3 text-xs">My account</Link><a href={phoneHref} className="btn-ghost-light min-h-10 px-3 text-xs"><Phone className="h-3.5 w-3.5" /> Call us</a><Link href="/book" className="btn-primary min-h-10 px-4 text-xs">Book cleaning</Link></div>
        <button type="button" className="grid h-10 w-10 place-items-center rounded-lg text-white hover:bg-white/8 lg:hidden" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls="mobile-navigation" aria-label={open ? "Close menu" : "Open menu"}>{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
      </div>
      {open ? <div id="mobile-navigation" className="container-page mt-2 overflow-hidden rounded-2xl border border-white/10 bg-brand-dark/98 p-2 shadow-elevated backdrop-blur-xl lg:hidden"><nav className="grid gap-1" aria-label="Mobile navigation">{links.map((link) => { const active = pathname === link.to; return <Link key={link.to} href={link.to} aria-current={active ? "page" : undefined} className={`rounded-xl px-3 py-3 text-sm font-bold ${active ? "bg-brand-lime text-brand-dark" : "text-white/78 hover:bg-white/8 hover:text-white"}`}>{link.label}</Link>; })}</nav><div className="mt-2 grid grid-cols-2 gap-2 border-t border-white/8 pt-2"><Link href="/portal/login" className="btn-ghost-light">My account</Link><a href={phoneHref} className="btn-ghost-light"><Phone className="h-4 w-4" /> Call us</a><Link href="/book" className="btn-primary col-span-2">Book now</Link></div></div> : null}
    </header>
  </>;
}
