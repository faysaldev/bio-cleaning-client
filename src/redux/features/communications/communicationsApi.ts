...
const api = baseApi.injectEndpoints({ endpoints: (builder) => ({
  ...
  submitPublicReview: builder.mutation<{submitted:boolean;redirectUrl?:string},{token:string;rating:number;comment?:string}>({ query:({token,...body})=>({url:`/reviews/public/${encodeURIComponent(token)}`,method:"POST",body}), transformResponse:(r:ApiResponse<any>)=>r.data }),
})});
