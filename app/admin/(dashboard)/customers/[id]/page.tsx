...
export default function CustomerDetailPage() {
  ...
  return <div className="space-y-6">
    <section className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><Link href={`/admin/quotes?customerId=${id}`} className="btn-primary"><FileText className="h-4 w-4" /> Create estimate</Link><button type="button" onClick={openEdit} className="btn-secondary"><Edit3 className="h-4 w-4" /> Edit profile</button></div>
    </section>
    ...
    <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ...
      <article className="surface p-5"><div className="flex items-center justify-between"><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-muted-foreground">Outstanding</p><ReceiptText className="h-4 w-4 text-brand-green" /></div><p className="mt-3 text-3xl font-extrabold tracking-[-0.04em] text-brand-dark">{money(data.outstandingBalance)}</p><p className="mt-1 text-xs text-muted-foreground">Open invoice balances</p></article>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_390px]">
      <div className="space-y-6">
        ...
        <section className="surface p-5 sm:p-6"><div className="flex items-center justify-between"><div><span className="editorial-kicker">Billing context</span><h2 className="mt-2 text-xl font-extrabold text-brand-dark">Invoices & balances</h2></div><ReceiptText className="h-5 w-5 text-brand-green" /></div>{data.invoices.length ? <div className="mt-5 space-y-2">{data.invoices.map((invoice) => <Link href="/admin/invoices" key={invoice._id} className="flex flex-col gap-2 rounded-xl border border-border p-4 transition hover:border-brand-green/30 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-extrabold text-brand-dark">{invoice.invoiceNumber}</p><p className="mt-1 text-[11px] text-muted-foreground">Issued {fmt(invoice.issuedAt)} · due {new Date(invoice.dueAt).toLocaleDateString()}</p></div><div className="flex flex-wrap items-center gap-3"><span className={`status-badge ${invoice.status === "PAID" ? "status-success" : invoice.status === "REFUNDED" ? "status-neutral" : "status-warning"}`}>{invoice.status.replace(/_/g, " ")}</span><div className="text-right"><p className="font-extrabold text-brand-dark">{invoice.currency} {invoice.total.toFixed(2)}</p>{invoice.amountDue > 0 ? <p className="text-[10px] font-bold text-destructive">{invoice.currency} {invoice.amountDue.toFixed(2)} due</p> : null}</div></div></Link>)}</div> : <p className="mt-5 text-sm text-muted-foreground">No invoices have been generated for completed work yet.</p>}</section>
